...
export default async function AdminDashboard({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const cards = [
    { label: t.admin.totalPalettes, value: stats.total.toLocaleString(), icon: Palette, color: "text-blue-600 bg-blue-50 dark:bg-blue-500/10" },
    { label: t.admin.published, value: stats.published.toLocaleString(), icon: Eye, color: "text-green-600 bg-green-50 dark:bg-green-500/10" },
    { label: t.admin.pendingReview2, value: stats.pending.toLocaleString(), icon: Clock, color: "text-amber-600 bg-amber-50 dark:bg-amber-500/10" },
    { label: t.admin.users, value: stats.users.toLocaleString(), icon: UsersIcon, color: "text-purple-600 bg-purple-50 dark:bg-purple-500/10" },
  ];
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        ...
        <p className="text-[14px] text-gray-500 dark:text-white/40 mt-1">{t.admin.overview}</p>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        <div className="bg-white dark:bg-white/[0.04] border border-gray-200/60 dark:border-white/[0.06] rounded-xl p-5">
          <h3 className="text-[15px] font-semibold text-gray-900 dark:text-white mb-4">{t.admin.quickStats}</h3>
          <div className="space-y-3">
            <div className="flex justify-between text-[14px]">
              <span className="text-gray-500 dark:text-white/50">{t.admin.totalLikes}</span>
              <span className="font-medium text-gray-900 dark:text-white">{stats.likes.toLocaleString()}</span>
            </div>
            <div className="flex justify-between text-[14px]">
              <span className="text-gray-500 dark:text-white/50">{t.admin.rejected}</span>
              <span className="font-medium text-gray-900 dark:text-white">{stats.rejected}</span>
            </div>
            <div className="flex justify-between text-[14px]">
              <span className="text-gray-500 dark:text-white/50">{t.admin.approvalRate}</span>
        ...
        <div className="bg-white dark:bg-white/[0.04] border border-gray-200/60 dark:border-white/[0.06] rounded-xl p-5">
          <h3 className="text-[15px] font-semibold text-gray-900 dark:text-white mb-4">{t.admin.quickActions}</h3>
          <div className="space-y-2">
            <a href={`/${locale}/admin/palettes`} className="block px-4 py-3 rounded-lg border border-gray-200/60 dark:border-white/[0.06] text-[14px] text-gray-700 dark:text-white/70 hover:bg-gray-50 dark:hover:bg-white/[0.04] transition-colors">
