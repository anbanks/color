...
import { Mail, Key, Send, Sparkles, BarChart3 } from "lucide-react";
...
export default function SettingsPage() {
  const [settings, setSettings] = useState<Settings>({ RESEND_API_KEY: "", RESEND_FROM_EMAIL: "", AI_PAUSED: "false", GA_MEASUREMENT_ID: "" });
  const [loading, setLoading] = useState(true);
  const [resendPending, startResendTransition] = useTransition();
  const [aiPending, startAiTransition] = useTransition();
  const [gaPending, startGaTransition] = useTransition();
    ...
    <div className="p-6 max-w-2xl">
      ...
      <div className="border border-gray-200 dark:border-white/10 rounded-xl p-5 mb-6">
        <div className="flex items-center gap-2.5 mb-4">
          ...
          <div>
            <h2 className="text-[15px] font-semibold text-gray-900 dark:text-white">Google Analytics</h2>
            <p className="text-[12px] text-gray-400 dark:text-white/40">GA4 Measurement ID — enables site-wide tracking and custom events</p>
          </div>
          <span className={`ml-auto text-[11px] font-medium px-2 py-0.5 rounded-full ${settings.GA_MEASUREMENT_ID ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-gray-100 text-gray-500 dark:bg-white/10 dark:text-white/50"}`}>
            {settings.GA_MEASUREMENT_ID ? "Connected" : "Not configured"}
          </span>
        </div>

        <div className="space-y-4">
          <div>
            <Label htmlFor="ga-id" className="text-[12px] text-gray-500 dark:text-white/40 flex items-center gap-1.5">
              <Key className="h-3 w-3" /> Measurement ID
            </Label>
            <Input
              id="ga-id"
              type="text"
            ...
            <p className="text-[11px] text-gray-400 mt-1">
              ...
              , then copy the Measurement ID (starts with <code className="font-mono">G-</code>).
            </p>
          </div>

          <div className="flex justify-end">
            <Button onClick={() => saveSettings(startGaTransition, "Google Analytics")} disabled={gaPending} className="min-w-[120px] cursor-pointer">
              {gaPending ? "Saving..." : "Save"}
            </Button>
          </div>
        </div>
      </div>
